...
struct MESSAGE_Sensorvalue* MESSAGE_createSensorValue(const struct MESSAGE_Data *data, const struct MESSAGE_Data *secret){
    ...
    memcpy(value,                data->data + offset, lvalue);              offset += lvalue;
    memcpy(accuracy,             data->data + offset, laccuracy);           offset += laccuracy;
    sscanf(value,    "%f", &v->value);
    sscanf(accuracy, "%f", &v->accuracy);
    if(accuracy == 0) v->accuracy = -666;
    return v;
}
